...
export default function ApplicationTracker({ sessionId, userProfile }) {
  ...
  const loadApplications = async () => {
    try {
      setLoading(true);
      const res = await getApplications(sessionId);
  ...
  const handleApply = async () => {
    ...
    try {
      setApplying(true);
      await createApplication(sessionId, selectedPolicy, formData);
    ...
    <div className="max-w-7xl mx-auto px-6 md:px-12" data-testid="application-tracker-section">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 border border-dashed border-border rounded-xl" data-testid="no-applications-msg">
          ...
          <p className="text-sm text-text-secondary">Start by chatting with Fin-ae to get personalized recommendations, then apply.</p>
        </div>
      ) : (
          ...
          {applications.map((app, i) => {
            ...
            return (
              <motion.div
                ...
                data-testid={`application-card-${app.application_id}`}
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-text-secondary mt-1">
                      ID: {app.application_id} | Applied: {new Date(app.created_at).toLocaleDateString()}
                    </p>
                  </div>
                    ...
                    {app.status_history.map((sh, j) => (
                      <div key={j} className="relative">
                        <div className={`absolute -left-[21px] top-0.5 w-3 h-3 rounded-full border-2 ${
                          j === app.status_history.length - 1 ? 'bg-primary border-primary' : 'bg-accent border-accent'
                        }`} />
                        ...
                        {sh.note && <p className="text-xs text-text-secondary/70 mt-0.5">{sh.note}</p>}
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
              <div>
                ...
                >
                  ...
                  {policies.map(p => (
                    <option key={p.policy_id} value={p.policy_id}>{p.name} - {p.provider}</option>
                  ))}
                </select>
              </div>
              {['name', 'email', 'phone', 'age', 'nationality', 'monthly_salary', 'employment_type'].map(field => (
                <div key={field}>
                  ...
                  <input
                    ...
                    type={field === 'age' || field === 'monthly_salary' ? 'number' : field === 'email' ? 'email' : 'text'}
